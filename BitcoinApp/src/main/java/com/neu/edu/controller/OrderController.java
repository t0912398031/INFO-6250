...
//		order.setStatus("Pending");
	...
	public ModelAndView match(@ModelAttribute("order") Order order, HttpServletRequest request)
			throws ClientException, OrderException, BitcoinException, EmailException {
	...
	public void match(Order b, Order s) throws ClientException, BitcoinException, OrderException, EmailException{
    ...
    */       
     ...
     * Send Email
    */    
//    if(buyer.getEmail()!=null) send(buyer.getEmail(), dealAmount, sellPrice);
//    if(seller.getEmail()!=null) send(seller.getEmail(), dealAmount, sellPrice);
    
	}
	
	public static void send(String email, int amount, double price) throws EmailException {
        Email e = new SimpleEmail();
        e.setHostName("smtp.googlemail.com");
        e.setSmtpPort(465);
        e.setAuthenticator(new DefaultAuthenticator("t0912398031@gmail.com", "tonyya66666google"));
        e.setSSLOnConnect(true);
        e.setCharset("UTF-8");
        e.setSubject("Bitcoin Transaction");
        e.setFrom("t0912398031@gmail.com");
        e.setMsg("Your order has been matched, price at:" + price + ", amount: " + amount + ".");
        e.addTo(email);
        e.send();
